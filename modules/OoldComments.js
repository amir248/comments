...
function commentsFn(app, jsonParser, cors, fs, id, importantBag) {
  ...
  console.log(importantBag.id + " " + importantBag[1] + " " + " <- importantBag");
...
console.log(importantBag.id+"____IMOPRTANT BBAG id");
    ...
    if(request.body.idea.site.pathname!=='/'){
      ...
      fs.access(`${fileOnPath}`, function(error){
          if (error) {
              console.log("Файл не найден 41");
              fs.mkdir(`${fileOnPath}`, err => {
               if(err) throw err; // не удалось создать папку
               console.log('Папка успешно создана 44');
            });
          } else {
              console.log("Файл найден 47");
          }
...
console.log(fileOnPathTwo+" <---fileOnPathTwo 476");
  ...
  app.post('/'+`${id[x]}`+'/',jsonParser,cors(),(request,response)=>{
    ...
    console.log(request.body.site.pathname +" < --- XXX 277 pathname");
    ...
    // console.log(importantBag.site+'___________7779');
      ...
      console.log(importantBag.id+" <important BAG 646" + " " + importantBag.pathname);
        let puthScript;
      if(importantBag.pathname=='/comments/'){
        // console.log(/\/$/.test(path));
        console.log('XXXXXXXXXXXXXXXXXXXXXXXX');
        fs.readFile(`${fileOnPathThatNeed}`, "utf8",
                  function(error,data){
                    console.log("Асинхронное чтение файла 496");
                    if(error) throw error; // если возникла ошибка
                    console.log(data);  // выводим считанные данные
